  ...
  <div class="panel-body">
  	<ul class="list-group">
         ...
         <?php   $mensajes = DB::table('mensaje')->where('fk_idgrupo', "$id_curso")->get(); ?>
                                    ...
                                    <?php $var= $mensaje->idmensaje; ?> 
                                        <li><a href="{!!URL::to('/formulario/mostrarforex').'/'.$var!!}"> {{ $mensaje->mensaje_titulo. "\n ".$mensaje->mensaje_src }}</a></li>
                                    @endforeach
   

     <a href="{!!URL::to('/usuario/create')!!}"><i class='fa fa-plus fa-fw'></i> Agregar</a>
      <a href="{!!URL::to('/formulario/createconfigex').'/'.$id_curso!!}" class="list-group-item" style="background-color: lightgreen;"> <i class="glyphicon glyphicon-record"></i>  <b>Formulario Exclusivo</b></a>
